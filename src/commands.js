...
async function handleCommand(ctx) {
  ...
  case 'speed': {
    const t = Date.now();
    await reply(`🏓 DENTSU MD V10 — *${Date.now() - t} ms*`);
    return true;
  }
  ...
  case 'runtime': {
    ...
    await reply(`✅ *DENTSU MD V10* is alive!\n⏱️ Uptime: *${h}h ${m}m ${s}s*`);
    return true;
  }
  ...
  case 'public': {
    ...
    config.MODE = 'public';
    await reply('✅ DENTSU MD V10 is now *Public Mode*.');
    return true;
  }
  ...
  case 'private': {
    if (!isOwner) return reply('❌ Owner only.');
    config.MODE = 'self';
    await reply('🔒 DENTSU MD V10 is now *Self Mode*.');
    return true;
  }
  ...
  case 'vv2': {
    ...
    if (mtype === 'imageMessage') await sock.sendMessage(from, { image: buf, caption: 'View-once revealed by DENTSU MD V10' }, { quoted: msg });
    else if (mtype === 'videoMessage') await sock.sendMessage(from, { video: buf, caption: 'View-once revealed by DENTSU MD V10' }, { quoted: msg });
...
╚═══════════════════╝`;
  ...
  case 'connect': {
    if (!isOwner) return reply('❌ Owner only.');
    const WEBSITE = process.env.WEBSITE || 'dentsu-md-v10.vercel.app/';
